...
```bash
javac -cp target/classes -d target/examples examples/*.java && java -cp target/classes:target/examples RunAll     # all 14, against a mock transport
java -cp target/classes:target/examples Example03Sale
```
